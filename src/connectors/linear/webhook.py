...
class LinearWebhookHandler:
    ...
    async def _handle_comment_event(
        ...
        if action == "create":
            ...
            return {"success": True, "action": "comment_created", "comment_id": comment_id}

        elif action == "update":
            logger.info(f"[LINEAR WEBHOOK] Updated comment {comment_id}")
            return {"success": True, "action": "comment_updated", "comment_id": comment_id}

        elif action == "remove":
            logger.info(f"[LINEAR WEBHOOK] Deleted comment {comment_id}")
            return {"success": True, "action": "comment_deleted", "comment_id": comment_id}
    ...
    async def _handle_project_event(
        ...
        if action == "create":
            ...
            return {"success": True, "action": "project_created", "project_id": project_id}

        elif action == "update":
            logger.info(f"[LINEAR WEBHOOK] Updated project: {project_name}")
            # TODO: Re-sync project
            return {"success": True, "action": "project_updated", "project_id": project_id}

        elif action == "remove":
            logger.info(f"[LINEAR WEBHOOK] Deleted project: {project_name}")
            # TODO: Soft-delete knowledge entries linked to this project
            return {"success": True, "action": "project_deleted", "project_id": project_id}
